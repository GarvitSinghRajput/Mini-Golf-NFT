...
public class PurchaseItemManager : MonoBehaviour
{
    ...
    [SerializeField] private TextMeshProUGUI cureveText;
    [SerializeField] private TextMeshProUGUI topSpinText;
    [SerializeField] private TextMeshProUGUI backSpinText;
    [SerializeField] private TextMeshProUGUI ballGuideText;
    ...
    //Control variables
    private Golf_Clubs _currentItemData;
    ...
    private void ActivateItemPanel(InventoryItem selectedItem)
    {
        ...
        powerText.text = selectedItem.GetData().Power.ToString();
        cureveText.text = selectedItem.GetData().Curve.ToString();
        topSpinText.text = selectedItem.GetData().TopSpin.ToString();
        backSpinText.text = selectedItem.GetData().BackSpin.ToString();
        ballGuideText.text = selectedItem.GetData().BallGuide.ToString();
        impactText.text = selectedItem.GetData().Impact.ToString();
        priceText.text = selectedItem.GetData().ClubPrice.ToString();
    ...
    {
        // 1. Build Metadata
        object metadata = MoralisTools.BuildMetadata(_currentItemData.Name, _currentItemData.Power.ToString(), _currentItemData.ImageUrl);
